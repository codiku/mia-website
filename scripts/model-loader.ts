...
async function updateSwaggerFile() {
  try {
    ...
    swaggerFileContent = swaggerFileContent.replace(
      /import \* as [^ ]+ from '@\/app\/api\/[^\/]+\/models';\n/g,
      ""
    );

    // Get all folders in the /app/api directory excluding 'auth'
    const apiFolders = fs
      .readdirSync(apiBasePath)
      .filter((folder: string) => folder !== "auth");
    ...
    for (const folder of apiFolders) {
      ...
      const importPath = `@/app/api/${folder}/models`;
    ...
    swaggerFileContent = swaggerFileContent.replace(
      zodModelsRegex,
      `const zodModels= {${newZodModelsEntries}}`
    );
